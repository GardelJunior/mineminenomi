...
public abstract class Quest {
	...
	public void startQuest(EntityPlayer player) {
		WyHelper.sendMsgToPlayer(player, EnumChatFormatting.GREEN + I18n.format("quest." + this.getQuestID() + ".name")
				+ " " + I18n.format(ID.LANG_GUI_QUESTS_STARTED));
	}
	...
	public void finishQuest(EntityPlayer player) {
		...
		WyNetworkHelper.sendTo(new PacketQuestCompleted(), (EntityPlayerMP) player);
		WyNetworkHelper.sendTo(new PacketQuestSync(questProps), (EntityPlayerMP) player);
		WyHelper.sendMsgToPlayer(player, EnumChatFormatting.GREEN + I18n.format("quest." + this.getQuestID() + ".name")
				+ " " + I18n.format(ID.LANG_GUI_QUESTS_COMPLETED));
	}
