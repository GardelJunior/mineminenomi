	...
	public static class ClientHandler implements IMessageHandler<PacketParticles, IMessage> {
		...
		public IMessage onMessage(PacketParticles message, MessageContext ctx) {
			...
			if (fx.contains("logiaEffect_")) {
				...
				EntityParticleFX logiaParticle = new EntityParticleFX(player.worldObj, ID.PARTICLE_ICON_MERA,
						message.posX + offsetX, (message.posY + 1) + offsetY, message.posZ + offsetZ, 0D, 0.01D, 0D)
								.setParticleAge(10);
				...
				if (devilFruit.equals("meramera"))
					...
					MainMod.proxy.spawnCustomParticles(player,
							logiaParticle.setParticleTexture(ID.PARTICLE_ICON_SUNA2));
				else if (devilFruit.equals("magumagu"))
					player.worldObj.spawnParticle(EnumParticleTypes.LAVA.getParticleName(), message.posX + offsetX,
							(message.posY + 1) + offsetY, message.posZ + offsetZ, 0D, 0D, 0D);
