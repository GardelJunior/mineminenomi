...
public class DevilFruitsHelper {
	...
	private static String[][] zoanModels = new String[][] { { "ushiushibison", "bison" },
			{ "toritoriphoenix", "phoenix" }, { "ushiushigiraffe", "giraffe" }, };
	...
	private static HashMap<String, List<Block>> createBlockRulesMap() {
		...
		map.put("core",
				Arrays.asList(new Block[] { Blocks.ice, Blocks.packed_ice, Blocks.stone, Blocks.grass, Blocks.dirt,
						Blocks.snow, Blocks.snow_layer, Blocks.sand, Blocks.sandstone, Blocks.sandstone_stairs,
						Blocks.wooden_door, Blocks.wooden_slab, Blocks.log, Blocks.log2, Blocks.carpet, Blocks.cake,
						ListMisc.Poison, ListMisc.DemonPoison, Blocks.torch, Blocks.redstone_torch,
						Blocks.redstone_wire, Blocks.cobblestone, Blocks.fence, Blocks.farmland, Blocks.fence_gate,
						Blocks.flower_pot, Blocks.clay, Blocks.gravel, ListMisc.SunaSand, ListMisc.WaxBlock }));

		map.put("air", Arrays.asList(new Block[] { Blocks.air }));

		map.put("foliage",
				Arrays.asList(new Block[] { Blocks.leaves, Blocks.leaves2, Blocks.waterlily, Blocks.double_plant,
						Blocks.yellow_flower, Blocks.red_flower, Blocks.vine, Blocks.brown_mushroom,
						Blocks.brown_mushroom_block, Blocks.red_mushroom, Blocks.red_mushroom_block, Blocks.tallgrass,
						Blocks.potatoes, Blocks.carrots, Blocks.cactus, Blocks.deadbush }));

		map.put("ores",
				Arrays.asList(new Block[] { Blocks.coal_ore, Blocks.coal_block, Blocks.diamond_ore,
						Blocks.diamond_block, Blocks.iron_ore, Blocks.iron_block, Blocks.lapis_ore, Blocks.lapis_block,
						Blocks.redstone_ore, Blocks.redstone_block, Blocks.gold_ore, Blocks.gold_block,
						ListMisc.KairosekiOre, ListMisc.KairosekiBlock }));

		map.put("liquids",
				Arrays.asList(new Block[] { Blocks.water, Blocks.flowing_water, Blocks.lava, Blocks.flowing_lava }));
	...
	public static void haoAttackEntities(EntityPlayer player) {
		...
		for (EntityLivingBase target : WyHelper.getEntitiesNear(player, 100)) {
			double userHakiExp = (props.getHardeningHakiExp() * 3) + (props.getImbuingHakiExp() * 3)
					+ (props.getObservationHakiExp() * 3);
			double targetHakiExp = 0;
			boolean hasBlindness = false;

			if (target instanceof EntityPlayer) {
				ExtendedEntityData propz = ExtendedEntityData.get(target);
				targetHakiExp = (propz.getHardeningHakiExp() * 3) + (propz.getImbuingHakiExp() * 3)
						+ (propz.getObservationHakiExp() * 3);
	...
	public static void sendShounenScream(EntityPlayer player, String ability, String soundId, int part) {
		if (MainConfig.enableAnimeScreaming)
			WyNetworkHelper.sendToAllAround(new PacketShounenScream(player.getCommandSenderName(),soundId, ability, part),
	...
	public static boolean canUseSwordsmanAbilities(EntityPlayer player) {
		ExtendedEntityData props = ExtendedEntityData.get(player);
		Ability sparClaw = AbilityProperties.get(player)
				.getAbilityFromName(ListAttributes.SPAR_CLAW.getAttributeName());
	...
	public static boolean isDevilFruitInWorld(World world, AkumaNoMi devilFruit) {
		...
		ExtendedWorldData worldData = ExtendedWorldData.get(world);
		String name = devilFruit.getUnlocalizedName().substring(5).replace("nomi", "").replace(":", "").replace(",", "")
				.replace("model", "");
	...
	public static boolean isAffectedByWater(EntityLivingBase entity) {
		...
		if (entity.isInsideOfMaterial(Material.water) || (entity.isWet() && !entity.isRiding()
				&& ((level1Block == Blocks.water || level1Block == Blocks.flowing_water)
						&& (level2Block == Blocks.water || level2Block == Blocks.flowing_water)))) {
			return true;
		}
	...
	public static boolean isNearbyKairoseki(EntityPlayer player) {
		if (WyHelper.isBlockNearby(player, 4, ListMisc.KairosekiBlock, ListMisc.KairosekiOre, ListMisc.KairosekiBars)
				|| ItemsHelper.hasKairosekiItem(player)
				|| player.inventory.hasItem(Item.getItemFromBlock(ListMisc.KairosekiBlock))
				|| player.inventory.hasItem(Item.getItemFromBlock(ListMisc.KairosekiOre))
				|| player.inventory.hasItem(Item.getItemFromBlock(ListMisc.KairosekiBars))
	 ...
	 * 
	 * core - basic blocks that will always be replaceable either due to their low
	 * value or commonness air - specific filter for the air block ores - self
	 * explanatory, also includes the compressed blocks for those ores foliage -
	 * things like flowers, vines and leaves liquids - water and lava all - all
	 * blocks restricted - removes some blocks that should never be replaced, has no
	 * use for add/ignore param
	 * 
	 * nogrief is used for abilities that should place blocks even if griefing is
	 * disabled, room or torikago for example
	 */

	public static boolean placeBlockIfAllowed(World world, int posX, int posY, int posZ, Block toPlace, int flag,
			String... rules) {
		return placeBlockIfAllowed(world, posX, posY, posZ, toPlace, 0, flag, rules);
	}

	public static boolean placeBlockIfAllowed(World world, int posX, int posY, int posZ, Block toPlace,
			String... rules) {
		return placeBlockIfAllowed(world, posX, posY, posZ, toPlace, 0, 3, rules);
	}

	public static boolean placeBlockIfAllowed(World world, int posX, int posY, int posZ, Block toPlace, int meta,
			int flag, String... rules) {
	...
	public static boolean isEntityInRoom(EntityLivingBase entity) {
		for (int i = -20; i < 20; i++)
			for (int j = -20; j < 20; j++)
				for (int k = -20; k < 20; k++) {
					if (entity.worldObj.getBlock((int) entity.posX + i, (int) entity.posY + j,
							(int) entity.posZ + k) == ListMisc.OpeMid)
						return true;
				}
