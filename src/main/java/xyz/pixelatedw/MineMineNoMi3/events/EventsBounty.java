			...
			if (MainConfig.enableAmbushes) {
				...
				if (props.isPirate() || props.isRevolutionary() && bountyLevel > 0) {
					if (player.worldObj.isDaytime()) {
						...
						boolean canSeeSky = player.worldObj.canBlockSeeTheSky((int) player.posX, (int) player.posY,
								(int) player.posZ);
						...
						if (player.ticksExisted % MainConfig.rateAmbushesSpawn == 0 && lightLevel > 0.9 && canSeeSky) {
							...
							if (bountyLevel <= 6) {
								...
								if (spawnType) {
									int lowLevelMobs = bountyLevel < 9 ? player.worldObj.rand.nextInt(10) + 5
											: player.worldObj.rand.nextInt(10) + 10;
									this.spawnAmbush(player, lowLevelMobs, EntityMarine.class);
					...
					if (!this.cachedPositions.containsKey(player))
						...
						if (flagPosX || flagPosZ) {
							if (BountyHelper.issueBountyForPlayer(event.player)) {
								...
								pkg.setLocationAndAngles(player.posX + WyMathHelper.randomWithRange(-10, 10),
										player.posY + 30, player.posZ + WyMathHelper.randomWithRange(-10, 10), 0, 0);
