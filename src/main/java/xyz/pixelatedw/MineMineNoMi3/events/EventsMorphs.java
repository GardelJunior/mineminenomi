...
public class EventsMorphs {
	...
	public void onArmorRendering(SetArmorModel event) {
		...
		Ability fullBodyHakiAbility = abilityProps
				.getAbilityFromName(ListAttributes.BUSOSHOKU_HAKI_FULL_BODY_HARDENING.getAttributeName());
		...
		if (fullBodyHakiAbility != null && fullBodyHakiAbility.isPassiveActive()) {
			GL11.glPushMatrix();
			{
				...
				float f2 = this.interpolateRotation(player.prevRenderYawOffset, player.renderYawOffset,
						event.partialRenderTick);
				float f3 = this.interpolateRotation(player.prevRotationYawHead, player.rotationYawHead,
						event.partialRenderTick);
				float f4 = this.handleRotationFloat(player, event.partialRenderTick);

				float f6 = player.prevLimbSwingAmount
						+ (player.limbSwingAmount - player.prevLimbSwingAmount) * event.partialRenderTick;
	...
	public void onEntityRendered(RenderLivingEvent.Pre event) {
		...
		if (event.entity instanceof EntityPlayer) {
			if (props.hasExtraEffects(ID.EXTRAEFFECT_ABAREHIMATSURI)) {
				if (event.entity.onGround) {
					Block block = event.entity.worldObj.getBlock((int) event.entity.posX, (int) event.entity.posY - 2,
							(int) event.entity.posZ);
					String texture = Blocks.dirt.getIcon(1, 0).getIconName();
					int blockTint = event.entity.worldObj
							.getBlock((int) event.entity.posX, (int) event.entity.posY - 2, (int) event.entity.posZ)
							.colorMultiplier(event.entity.worldObj, (int) event.entity.posX,
									(int) event.entity.posY - 2, (int) event.entity.posZ);
		 ...
		 * GL11.glPushAttrib(GL11.GL_ALL_ATTRIB_BITS); GL11.glEnable(GL11.GL_LIGHTING);
		 * GL11.glClearStencil(0); GL11.glClear(GL11.GL_STENCIL_BUFFER_BIT);
		 * GL11.glEnable(GL11.GL_STENCIL_TEST); GL11.glStencilFunc(GL11.GL_ALWAYS, 1,
		 * 0xFFFF); GL11.glStencilOp(GL11.GL_KEEP, GL11.GL_KEEP, GL11.GL_REPLACE);
		 * GL11.glPolygonMode(GL11.GL_FRONT_AND_BACK, GL11.GL_FILL);
		 * GL11.glColor3f(1.0f, 1.0f, 1.0f); // Render original. //new
		 * RenderZoanMorph(new ModelBiped(), "null", 2).doRender(event.entity, event.x,
		 * event.y, event.z, 0, 0); GL11.glDisable(GL11.GL_LIGHTING);
		 * GL11.glStencilFunc(GL11.GL_NOTEQUAL, 1, 0xFFFF);
		 * GL11.glStencilOp(GL11.GL_KEEP, GL11.GL_KEEP, GL11.GL_REPLACE);
		 * GL11.glLineWidth(6.5f); GL11.glPolygonMode(GL11.GL_FRONT, GL11.GL_LINE);
		 * GL11.glColor4f(1.0F, 1.0F, 0.0F, 1.0F); GL11.glDisable(GL11.GL_TEXTURE_2D);
		 ...
		 * GL11.glPopAttrib(); //event.setCanceled(true); }
		 */
	...
	public void onEntityConstructing(EntityJoinWorldEvent event) {
		if (event.entity instanceof EntityPlayer) {
			...
			if (!props.getZoanPoint().toLowerCase().equals("n/a")
					&& !props.getZoanPoint().toLowerCase().equals("yomi")) {
	...
	public void morphHandRendering(RenderHandEvent event) {
		...
		Ability hotBoilingSpecial = abilityProps
				.getAbilityFromName(ListAttributes.HOT_BOILING_SPECIAL.getAttributeName());
		Ability hardeningBuso = abilityProps
				.getAbilityFromName(ListAttributes.BUSOSHOKU_HAKI_HARDENING.getAttributeName());
		Ability fullBodyHardeningBuso = abilityProps
				.getAbilityFromName(ListAttributes.BUSOSHOKU_HAKI_FULL_BODY_HARDENING.getAttributeName());
		...
		GL11.glPushMatrix();
		{
			 ...
			 * (float) mc.displayHeight, 0.20F, (float)
			 * (mc.gameSettings.renderDistanceChunks * 16) * 2.0F);
			 * GL11.glMatrixMode(GL11.GL_MODELVIEW); GL11.glLoadIdentity();
			 * RenderHelper.enableStandardItemLighting();
			 * Minecraft.getMinecraft().entityRenderer.enableLightmap(0); int i2 =
			 * mc.theWorld.getLightBrightnessForSkyBlocks(MathHelper.
			 * floor_double(player.posX), MathHelper.floor_double(player.posY),
			 * MathHelper.floor_double(player.posZ), 0); int j = i2 % 65536; int k = i2 /
			 * 65536; OpenGlHelper.setLightmapTextureCoords(OpenGlHelper. lightmapTexUnit,
			 * (float)j / 1.0F, (float)k / 1.0F); GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
			 */

			/*
			 * if (mc.gameSettings.thirdPersonView == 0 &&
			 * !mc.renderViewEntity.isPlayerSleeping() && !mc.gameSettings.hideGUI) {
			 * GL11.glRotated(180, 0, 0, 1); GL11.glScaled(0.05, 0.05, 0.05);
			 ...
			 * WyHelper.getFancyName("baribarinopistol") + ".png")); Tessellator tessellator
			 ...
			 * tessellator.addVertexWithUV(x , y , 0, 0.0, 0.0); tessellator.draw();
			 * tessellator.startDrawingQuads(); tessellator.addVertexWithUV(x + u , y , 0,
			 * 1.0, 0.0); tessellator.addVertexWithUV(x + u , y + v , 0, 1.0, 1.0);
			 * tessellator.addVertexWithUV(x , y + v , 0, 0.0, 1.0);
			 * tessellator.addVertexWithUV(x , y , 0, 0.0, 0.0); tessellator.draw(); }
			 */
		}
