	...
	public void processCommand(ICommandSender sender, String[] str) {
		/*if (str.length >= 1) {
			...
			if (str[0].equalsIgnoreCase("package")) {
				...
				toSpawn.setLocationAndAngles(player.posX + WyMathHelper.randomWithRange(-10, 10), player.posY + 30,
						player.posZ + WyMathHelper.randomWithRange(-10, 10), 0, 0);
				...
				player.worldObj.setBlock((int) player.posX, (int) player.posY + 1, (int) player.posZ,
						ListMisc.CustomSpawner);
				...
				if (str[1].equalsIgnoreCase("start")) {
					if (!player.worldObj.isRemote)
						new TeleporterScenarioArena((WorldServer) player.worldObj).teleport(player,
								ID.SCENARIO_ROMANCEDAWN_CAPTAINMORGAN);
				} else if (str[1].equalsIgnoreCase("end")) {
					if (!player.worldObj.isRemote)
						new TeleporterScenarioArena((WorldServer) player.worldObj).endScenario(player,
								ID.SCENARIO_ROMANCEDAWN_CAPTAINMORGAN);
				}
				...
				// QuestManager.instance().startQuest(player,
				// questsPool[player.getRNG().nextInt(questsPool.length)]);
				QuestManager.getInstance().startQuest(player, ListQuests.bountyLowLevel01);
				...
				StructureMarineLargeBase.build(WySchematicHelper.load("marineLargeBase"), player.worldObj,
						(int) player.posX, (int) player.posY, (int) player.posZ,
						player.worldObj.getBiomeGenForCoordsBody((int) player.posX, (int) player.posZ));
			else if (str[0].equalsIgnoreCase("banditbase"))
				StructureBanditSmallBase.build(WySchematicHelper.load("banditBase"), player.worldObj, (int) player.posX,
				...
				StructureLargeShip.populate((int) player.posX, (int) player.posY, (int) player.posZ, player.worldObj,
						sch.getName());
